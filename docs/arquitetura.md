...
3. Ao buscar postos, o controlador do localizador obtém a origem por endereço ou GPS e seleciona os candidatos do cadastro pela distância geográfica.
4. O serviço de mapas consulta as rotas. Em caso de falha, o controlador usa a estimativa de distância e mantém essa indicação no resultado.
...
- **Persistência:** `services/armazenamento.js` lê e grava os parâmetros. Ao ler cadastros antigos, soma `inpCapacidade1` e `inpCapacidade2` no novo campo `inpCapacidade`. Um valor já salvo no campo novo tem prioridade e não é somado novamente. O controlador informa falhas sem impedir o uso da página.
...
- Para substituir o serviço de mapas, preserve os contratos de `services/mapas.js`: origem com coordenadas e nome; rota com distância em quilômetros e tempo em minutos.
