...
Para usar o banco por HTTP, execute o servidor Node.js; hospedagem apenas estática não fornece `/api/postos`. O servidor local aceita `/index.html` como endereço alternativo, sem duplicar o HTML no projeto. A abertura direta do HTML por arquivo usa o cadastro de referência sem acessar o banco.
