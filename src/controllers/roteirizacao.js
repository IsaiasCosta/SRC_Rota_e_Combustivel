(function (app) {
    ...
    function editarLoja(id) {
        const loja = app.lojas.find(item => String(item.id) === String(id));
        ...
        lojaEmEdicao = loja.id;

        el('lojaNome').value = loja.Nome || '';
        el('lojaMarca').value = loja.Marca || '';
        el('lojaEndereco').value = loja.Endereço || '';
        el('lojaCidade').value = loja.Cidade || '';
        el('lojaEstado').value = loja.Estado || '';
        el('lojaLat').value = loja.lat ?? '';
        el('lojaLon').value = loja.lon ?? '';
        el('lojaLinkMaps').value = loja.linkMaps || '';

        const campoId = el('lojaIdEdicao');
        if (campoId) campoId.value = loja.id;
    ...
    async function cadastrarLoja(event) {
        ...
        try {
            ...
            statusCadastro.textContent = idEmEdicao != null
                ? 'Atualizando loja...'
                : 'Salvando loja...';

            const resultado = idEmEdicao != null
                ? await app.services.lojas.atualizar(idEmEdicao, dados)
                : await app.services.lojas.cadastrar(dados);
            ...
            const botaoSalvar = el('btnSalvarLoja');
            if (botaoSalvar) botaoSalvar.textContent = 'Salvar loja';
            ...
            statusCadastro.textContent = error.message;
        } finally {
    ...
    function alternarCadastroLoja(tipo) {
        ...
        // Se a opção clicada já estiver aberta, fecha.
        // Caso contrário, abre a opção escolhida.
        const deveAbrir = abrirFormulario
            ? !formulario.open
            : !importacao.open;

        formulario.open = abrirFormulario && deveAbrir;
        importacao.open = !abrirFormulario && deveAbrir;

        el('btnAbrirFormularioLoja').setAttribute(
            'aria-expanded',
            String(formulario.open)
        );

        el('btnAbrirImportacaoLoja').setAttribute(
            'aria-expanded',
            String(importacao.open)
        );
    ...
    function inicializar() {
        ...
        el('listaLojas').addEventListener('click', event => {
            const botao = event.target.closest('.btn-editar-loja');

            if (!botao) return;

            event.preventDefault();
            editarLoja(botao.dataset.lojaId);
        });
