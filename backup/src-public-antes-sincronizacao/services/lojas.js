(function (app) {
    ...
    function dadosLoja(dados) {
        return {
            nome: dados.Nome || dados.nome, marca: dados.Marca || dados.marca || null,
            ...
            latitude: numero(dados.lat ?? dados.latitude),
            longitude: numero(dados.lon ?? dados.longitude),
   ...
   async function atualizar(id, dados) {
            if (id == null || id === '') {
                throw new Error('Selecione uma loja para editar.');
            }

            const atualizadas = await requisicao('lojas', {
                method: 'PATCH',
                query: `id=eq.${encodeURIComponent(id)}&select=id`,
                headers: {
                    Prefer: 'return=representation'
                },
                body: JSON.stringify(dadosLoja(dados))
            });

            if (!Array.isArray(atualizadas) || atualizadas.length !== 1) {
                throw new Error(
                    'A atualização não foi confirmada. Verifique se a loja existe e se sua conta tem permissão.'
                );
            }

            return {
                erros: [],
                lojas: await carregar()
            };
        }
    ...
    app.services.lojas = { carregar, cadastrar, atualizar, importarCSV };
